  ...
  return (
    <>
      {winner && (
        <section className="bg-wc-gold border-b-[3px] border-wc-ink relative overflow-hidden">
          ...
          <div className="relative mx-auto max-w-7xl px-4 py-10 md:py-14 text-center">
            ...
            <h2 className="font-display text-4xl sm:text-5xl md:text-8xl leading-[0.95] mt-2 text-wc-ink break-words">
              <span className="text-4xl sm:text-5xl md:text-7xl mr-2 align-middle">{winner.team.flag}</span>
              {winner.team.name.toUpperCase()}
            </h2>
            <p className="font-display text-xl sm:text-2xl md:text-4xl mt-2 text-wc-magenta">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-8 md:py-16 grid md:grid-cols-[1.4fr_1fr] gap-6 md:gap-8 items-center">
          <div className="chunky-card bg-wc-cream/95 p-5 md:p-8 order-2 md:order-1">
            ...
            <div className="mt-5 flex flex-wrap gap-2 md:gap-3">
              ...
              <Link href="/predictions" className="chunky-btn bg-wc-gold text-wc-ink px-4 md:px-5 py-2 font-bold text-sm md:text-base">Play predictions</Link>
              <Link href="/groups" className="chunky-btn bg-white text-wc-ink px-4 md:px-5 py-2 font-bold text-sm md:text-base">Group tables</Link>
            </div>
          </div>
      ...
      <section className="bg-wc-ink text-wc-cream border-b-[3px] border-wc-ink">
        <div className="mx-auto max-w-7xl px-4 py-5 md:py-6 flex flex-wrap items-center gap-4 md:gap-6">
          <div className="min-w-0">
            <div className="text-[10px] uppercase tracking-[0.3em] text-wc-gold font-bold">Counting down to</div>
            <div className="font-display text-2xl md:text-3xl truncate">
              {opener.home.code ? `${teamName(opener.home.code)} v ${teamName(opener.away.code ?? "")}` : "Opening match"}
            </div>
          </div>
          <div className="md:ml-auto w-full md:w-auto"><Countdown iso={opener.kickoffUtc} /></div>
        </div>
      </section>
